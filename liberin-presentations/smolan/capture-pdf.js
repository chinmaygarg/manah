#!/usr/bin/env node
/**
 * Liberin Capabilities Deck — PDF exporter.
 *
 ...
 *
 * Usage:
 *   node capture-pdf.js                  # screenshots/ + Liberin_Capabilities_Deck.pdf
 *   node capture-pdf.js --out my.pdf     # custom PDF filename
 *   node capture-pdf.js --port 8891      # custom local server port
...
const PORT = parseInt(getArg('port', '8891'), 10)
const OUT_PDF = path.resolve(HERE, getArg('out', 'Liberin_Capabilities_Deck.pdf'))
...
async function capture() {
  ...
  await page.addStyleTag({
    content: `
      #fullscreen-btn, .reveal .controls, .reveal .progress, .reveal .slide-number {
